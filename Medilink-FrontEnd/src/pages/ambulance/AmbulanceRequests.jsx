...
export default function AmbulanceRequests() {
  const { fetchAmbulanceRequests, updateRequestStatus } = useData();
  ...
  const loadRequests = async () => {
    try {
      setLoading(true);
      const data = await fetchAmbulanceRequests();
      setRequests(data);
    } catch (error) {
      console.error(error);
  ...
  if (loading) {
    return <div className="empty-state"><h3>Loading requests...</h3></div>;
  }
            ...
            <div key={req.id} className="request-card">
              ...
              {req.status === 'pending' && (
                <div className="request-actions">
                  <button className="btn btn-accent btn-sm" onClick={() => handleRespond(req.id, 'accepted')}>
                    <CheckCircle size={14} /> Accept
                  </button>
                  <button className="btn btn-danger btn-sm" onClick={() => handleRespond(req.id, 'rejected')}>
