...
import { AlertCircle, Clock, MapPin, Hospital, Activity, UserPlus } from 'lucide-react';
...
export default function EmergencyCases() {
  ...
  const handleDoctorAssign = (reqId, doctorId) => {
    if(!doctorId) return;
          ...
          <table style={{ width: '100%', textAlign: 'left', borderCollapse: 'collapse' }}>
            <thead style={{ background: 'var(--bg-secondary)' }}>
              <tr>
                ...
                <th style={{ padding: '16px 24px', fontWeight: 600, color: 'var(--text-secondary)' }}>{isDoctor ? 'Priority Override' : 'Assigned Doctor'}</th>
                    ...
                    <td style={{ padding: '16px 24px' }}>
                      {isDoctor ? (
                         <select 
                          ...
                          onChange={(e) => { toast.success(`Priority changed to ${e.target.value}`) }}
                        >
                        ...
                        <select 
                          className="form-input" 
                          style={{ padding: '6px 12px', height: 'auto', width: '160px', fontSize: '0.85rem' }}
