...
import { initialHospitals, initialAmbulances, initialPatients, initialAdmins, initialDoctors } from '../data/mockData';
...
export function AuthProvider({ children }) {
  ...
  const login = (role, credentials) => {
    ...
    if (role === 'admin') {
      ...
      // Check if it's a doctor logging in through the admin tab
      const doctor = initialDoctors.find(
        d => d && d.email.toLowerCase() === credentials.email.toLowerCase() && d.password === credentials.password
      );
      if (doctor) {
        setUser({ role: 'doctor', doctorId: doctor.id, name: doctor.name, email: doctor.email });
        return { success: true };
      }
